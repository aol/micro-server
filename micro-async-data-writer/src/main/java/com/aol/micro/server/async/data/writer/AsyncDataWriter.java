...
public class AsyncDataWriter<T> implements DataWriter<T> {
	...
	public static <T> AsyncDataWriter<T> asyncDataWriter(Executor executorService,ManifestComparator<T> comparator,EventBus bus){
		return new AsyncDataWriter<>(executorService,comparator,bus);
	}
	...
	public FutureW<T> loadAndGet(){
		...
		return FutureW.ofSupplier(()-> Tuple.tuple(comparator.load(),comparator.getData()),executorService)
				.peek(t->bus.post(SystemData.<String,String>builder()
				 ...
				 .errors(0)
				 .processed(t.v1?1:0)
				 .build())) //add recover option here also with cyclops-react 1.0.0-final
				.map(t->t.v2);
	}
	...
	public FutureW<Void> saveAndIncrement(T data){
		String correlationId = ""+System.currentTimeMillis() + ":" + r.nextLong();
		Supplier<MapX<String, String>> dataMap=()->MapX.fromMap(HashMapBuilder.map(MANIFEST_COMPARATOR_DATA_WRITER_KEY,comparator.toString()).build());
		return FutureW.<Void>ofSupplier(()-> {
			comparator.saveAndIncrement(data);
			return null;
		},executorService).peek(t->bus.post(SystemData.<String,String>builder()
				 .correlationId(correlationId)
				 .dataMap(dataMap.get())
				 .errors(0)
				 .processed(1)
